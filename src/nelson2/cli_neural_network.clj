...
(defn neural-command [command]
  (cond
    ...
    (= command "@remove-neuron") (do (print "Enter the neuron ID (without :) :- ") (flush) (neural-processes/remove-neuron (keyword (read-line))))
    (= command "@neural-list") (println (str "Neurons :- " (keys @brain/neural-cluster)))
    (= command "@clean-garbage") (neural-processes/collect-garbage)
    ...
    (= command "@quit") (do  (utility/save-neurons @brain/neural-cluster) (println "Saving neurons")
                             (reward-cluster/save-neurons @reward-cluster/personality) (println "Saving reward neurons") (shutdown-agents)
                            (println "Shutting down") (log/log "All agents shutdown"))
